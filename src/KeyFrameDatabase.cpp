...
vector<KeyFrame*> KeyFrameDatabase::DetectLoopCandidates(KeyFrame* pKF, float minScore)
{
    ...
    // 步骤1：找出和当前帧具有公共单词的所有关键帧（不包括与当前帧链接的关键帧）
    {
        ...
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)
        {
            ...
            list<KeyFrame*> &lKFs =   mvInvertedFile[vit->first];//通过words的ID得到包含了这个words的所有关键帧 关键帧和当前共享的词汇越多越可能是回环
    ...
    list<pair<float,KeyFrame*> > lScoreAndMatch;//得分，和得分对应的关键帧，这个集合里的都是大于一个阈值minScore的
    ...
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        if((*lit)->mnLoopWords>maxCommonWords)
            maxCommonWords=(*lit)->mnLoopWords;//计算最高得分
    }

    int minCommonWords = maxCommonWords*0.8f;//最低得分
    ...
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        ...
        if(pKFi->mnLoopWords>minCommonWords)
        {
            ...
            float si = mpVoc->score(pKF->mBowVec,pKFi->mBowVec);//计算候选帧的相似度

            pKFi->mLoopScore = si;//更新得分
            if(si>=minScore)
                lScoreAndMatch.push_back(make_pair(si,pKFi));//大于最小得分就加入到这个集合中
        }
    }
    ...
    list<pair<float,KeyFrame*> > lAccScoreAndMatch;//记录和刚刚选出的候选帧相连的关键帧里相似度最高的帧
    float bestAccScore = minScore;//从新计算准确度得分和
    ...
    for(list<pair<float,KeyFrame*> >::iterator it=lScoreAndMatch.begin(), itend=lScoreAndMatch.end(); it!=itend; it++)
    {
        ...
        vector<KeyFrame*> vpNeighs = pKFi->GetBestCovisibilityKeyFrames(10);//得到与刚刚匹配到的回环候选帧相连的共视程度最高的关键帧
        ...
        {
            KeyFrame* pKF2 = *vit;
            if(pKF2->mnLoopQuery==pKF->mnId && pKF2->mnLoopWords>minCommonWords)//检查这组的关键帧是不是候选帧，并且匹配的单词数大于最小的匹配单词数
            {
                ...
                {
                    pBestKF=pKF2;//更新最佳候选帧和分数
                    bestScore = pKF2->mLoopScore;
                }
            }
        }

        lAccScoreAndMatch.push_back(make_pair(accScore,pBestKF));//会重复包含同一个帧
        if(accScore>bestAccScore)// 记录所有组中组得分最高的组
            bestAccScore=accScore;//更新最高累计得分
    }

    // Return all those keyframes with a score higher than 0.75*bestScore
    float minScoreToRetain = 0.75f*bestAccScore;//保留大于0.75倍累加分数的关键帧
